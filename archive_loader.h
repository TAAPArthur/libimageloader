...
int archive_open(ImageLoaderData* parent) {
    ...
    if(archive_read_open_fd(state->a, parent->fd, 4096) == ARCHIVE_OK) {
...
ImageLoaderData* archive_next(ImageLoaderData* parent) {
    ...
    while (archive_read_next_header(state->a, &state->entry) == ARCHIVE_OK) {
        ...
        return createImageLoaderData(memory_fd, name, IMG_DATA_KEEP_OPEN | IMG_DATA_FREE_NAME,
